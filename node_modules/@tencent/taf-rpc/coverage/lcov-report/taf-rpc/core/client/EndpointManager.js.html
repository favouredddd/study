...
<html lang="en">
<head>
    <title>Code coverage report for taf-rpc\core\client\EndpointManager.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../../../index.html">all files</a> / <a href="index.html">taf-rpc/core/client/</a> EndpointManager.js
    </h1>
    <div class='clearfix'>
      ...
      <div class='fl pad1y space-right2'>
        ...
        <span class='fraction'>74/112</span>
      </div>
      <div class='fl pad1y space-right2'>
      ...
      <div class='fl pad1y space-right2'>
        <span class="strong">77% </span>
        <span class="quiet">Lines</span>
        <span class='fraction'>154/200</span>
      </div>
    </div>
  </div>
...
337
338
339</td><td class="line-coverage quiet"><span class="cline-any cline-yes">1×</span>
...
<span class="cline-any cline-yes">1×</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">var Registry     = require("@tencent/taf-registry");
...
<span class="cstat-no" title="statement not covered" >        this._consistentHash = new ConsistentHash();</span>
...
&nbsp;
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if(this._consistentHash){
<span class="cstat-no" title="statement not covered" >        this._consistentHash.addEndpoints(endpointInfoArr);</span>
    }
};
&nbsp;
...
{
    var self = this;
    self._consistentHash = new ConsistentHash();
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if(endpointInfoArr.length){
...
EndpointManager.prototype._doEndpointsException = function (data) {
    <span class="missing-if-branch" title="else path not taken" >E</span>if(data &amp;&amp; data.response &amp;&amp; data.response.error){
        console.error("[TAF][EndpointManager doEndpoints, exception ,return:",data.response.error.code," ,objname:", this._worker.name," ,setname:",this._setname);
    }
...
<span class="cstat-no" title="statement not covered" >        console.error("[TAF][EndpointManager doEndpoints exception ,return:",result.response.return," ,objname:", this._worker.name," ,setname:",this._setname);</span>
...
<span class="cstat-no" title="statement not covered" >        console.error("[TAF][EndpointManager doEndpoints, callback activeEps is empty,objname:", this._worker.name," ,setname:",this._setname);</span>
...
&nbsp;
    //01 将新的节点接入到活动列表中
    var actives   = [], i = 0, newEndpoints = [], inactiveEndpoints = [],vNodeNumber = this._iVnodeNumber;
    ...
    for (i = 0, len = endpoints.value.length; i &lt; len; i++) {
        ...
        newEndpoint.iAuthType = endpoints.value[i].authType || 0;//默认不需要鉴权
&nbsp;
...
<span class="cstat-no" title="statement not covered" >            inactiveEndpoints.push(endpoint);</span>
            //从hashring中删除
<span class="cstat-no" title="statement not covered" >            if(this._consistentHash){</span>
<span class="cstat-no" title="statement not covered" >                this._consistentHash.removeEndpoints([endpoint]);</span>
            }
<span class="cstat-no" title="statement not covered" >            i--;</span>
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if(newEndpoints.length &amp;&amp; this._consistentHash){
<span class="cstat-no" title="statement not covered" >        this._consistentHash.addEndpoints(newEndpoints);</span>
    }
    //从一致性哈希中删除节点
    <span class="missing-if-branch" title="if path not taken" >I</span>if(inactiveEndpoints.length &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >this._consistentHash)</span>{
<span class="cstat-no" title="statement not covered" >        this._consistentHash.removeEndpoints(inactiveEndpoints);</span>
    }
    //每次adapter列表添加新成员之后sort一次，保证所有节点的adapter顺序一致，避免出现使用hashcode时节点仍然没有分发到同一节点的情况
    <span class="missing-if-branch" title="else path not taken" >E</span>if(newEndpoints.length){
        this._adapters.sort(function(adapter1, adapter2){
            return adapter1.endpoint.sHost &gt; adapter2.endpoint.sHost ? <span class="branch-0 cbranch-no" title="branch not covered" >-1 </span>: 1;
        });
    }
...
<span class="cstat-no" title="statement not covered" >        console.error("[TAF][EndpointManager doEndpoints, requesting but timeout ,objname:", this._worker.name," ,setname:",this._setname);</span>
...
<span class="cstat-no" title="statement not covered" >                    endpoints.splice(0, 1);</span>
