...
<html lang="en">
<head>
    <title>Code coverage report for taf-rpc\core\client\AdapterProxy.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../../../index.html">all files</a> / <a href="index.html">taf-rpc/core/client/</a> AdapterProxy.js
    </h1>
    <div class='clearfix'>
      <div class='fl pad1y space-right2'>
        <span class="strong">58.72% </span>
        <span class="quiet">Statements</span>
        <span class='fraction'>128/218</span>
      </div>
      <div class='fl pad1y space-right2'>
        <span class="strong">36.51% </span>
        <span class="quiet">Branches</span>
        <span class='fraction'>46/126</span>
      </div>
      <div class='fl pad1y space-right2'>
      ...
      <div class='fl pad1y space-right2'>
        <span class="strong">57.75% </span>
        <span class="quiet">Lines</span>
        <span class='fraction'>123/213</span>
      </div>
    </div>
  </div>
...
421
422
423</td><td class="line-coverage quiet"><span class="cline-any cline-neutral">&nbsp;</span>
...
var TafMonitor     = require("@tencent/taf-monitor");
...
<span class="cstat-no" title="statement not covered" >        this._pTrans = new UDPTransceiver(this, this._endpoint);</span>
    }
};
AdapterProxy.prototype._getAuthInfo = <span class="fstat-no" title="function not covered" >function () {</span>
<span class="cstat-no" title="statement not covered" >    return this._worker._getAuthInfo();</span>
}
...
AdapterProxy.prototype.destroy = function () {
    ...
    //此时节点已经从endpoint列表移除，不会再接收新的请求，只会等待已发送的请求返回并处理
    setTimeout(function () {
...
&nbsp;
    $reqMessage.clearTimeout();
    $reqMessage.adapter.finishInvoke(TafError.CLIENT.REQUEST_TIMEOUT, $reqMessage);
    $reqMessage.LocalEndpoint  = this._pTrans.LocalEndpoint;
    $reqMessage.promise.reject(new RpcCallError({request:$reqMessage, response:undefined, error:{code:TafError.CLIENT.REQUEST_TIMEOUT, message:"call remote server timeout(remote server no response)"}}));
};
&nbsp;
// 发送积压的数据
// 当前代理类重新连接或者第一次连接上对端服务之后，回调该函数
// bOnlySelf:鉴权成功时，应该只处理当前adapter的积压请求
AdapterProxy.prototype.doInvoke = function (bOnlySelf) {
    // 遍历未发送的请求
    var self = this;
    self._pTimeoutQueueN.forEach(function (key) {
        var reqMessage = self._pTimeoutQueueN.pop(key, true);
        <span class="missing-if-branch" title="else path not taken" >E</span>if (self._pTrans.sendRequest(reqMessage.request)) {
...
&nbsp;
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if(!bOnlySelf) self._worker.doInvoke();
};
...
<span class="cstat-no" title="statement not covered" >    $reqMessage.adapter.finishInvoke(TafError.CLIENT.ADAPTER_NOT_FOUND, $reqMessage);</span>
<span class="cstat-no" title="statement not covered" >    $reqMessage.promise.reject(new RpcCallError({request:$reqMessage, response:undefined, error:{code:TafError.CLIENT.ADAPTER_NOT_FOUND, message:"call remote server error(send again error)"}}));</span>
};
&nbsp;
AdapterProxy.prototype.invoke = function ($reqMessage) {
    $reqMessage.adapter = this;
    $reqMessage.RemoteEndpoint = this._endpoint;
    //不需要鉴权的，或者鉴权已成功的请求，可直接发送，否则加入队列
...
<span class="cstat-no" title="statement not covered" >    this._pTrans.close();</span>
};
&nbsp;
// 判断当前的接口代理实例是否可用
...
<span class="cstat-no" title="statement not covered" >        if (!this._pTrans.isValid()) {</span>
<span class="cstat-no" title="statement not covered" >            this._pTrans.reconnect();</span>
        }
<span class="cstat-no" title="statement not covered" >        return this._pTrans.hasConnected() || this._pTrans.isConnecting();</span>
    }
...
&nbsp;
    //如果还没有连接过，那么就重新连接
    <span class="missing-if-branch" title="else path not taken" >E</span>if (!this._pTrans.isValid()) {
...
<span class="cstat-no" title="statement not covered" >            console.info("[TAF][AdapterProxy::finishInvoke(bool), ", this._worker.name, ", " ,this._endpoint.toString(), ", retry fail]");</span>
        }
<span class="cstat-no" title="statement not covered" >        return ;</span>
    }
&nbsp;
...
&nbsp;
    if ($iResultCode !== 0) { //调用失败
        this._timeoutInvoke++;
        <span class="missing-if-branch" title="else path not taken" >E</span>if (this._frequenceFailInvoke === 0) {
...
&nbsp;
        ...
        <span class="missing-if-branch" title="if path not taken" >I</span>if (this._frequenceFailInvoke &gt;= this._worker._checkTimeoutInfo.frequenceFailInvoke &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >nowTime &gt;= this._frequenceFailTime)</span> {
<span class="cstat-no" title="statement not covered" >            this._setInactive();</span>
        }
    } else { //调用成功
        this._frequenceFailInvoke = 0;
...
&nbsp;
    ...
    if (nowTime &gt; this._nextFinishInvokeTime) {
        ...
        <span class="missing-if-branch" title="if path not taken" >I</span>if ($iResultCode !== 0 &amp;&amp; this._timeoutInvoke &gt;= this._worker._checkTimeoutInfo.minTimeoutInvoke &amp;&amp; <span class="branch-2 cbranch-no" title="branch not covered" >this._timeoutInvoke &gt;= parseInt(this._worker._checkTimeoutInfo.radio * this._totalInvoke))</span> {
<span class="cstat-no" title="statement not covered" >            this._setInactive();</span>
        } else {
            this._totalInvoke   = 0;
            this._timeoutInvoke = 0;
...
<span class="cstat-no" title="statement not covered" >        process.send({</span>
    ...
    if($reqMessage.request.property &amp;&amp; $reqMessage.request.property.packetType === 1)
        packetType = 1;
&nbsp;
    //如果是在正式环境，则上报统计信息(单向调用不需要上报), wup由于在服务端上报，客户端也不做上报
    <span class="missing-if-branch" title="if path not taken" >I</span>if (process.env.TAF_MONITOR &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >packetType !== 1 </span>&amp;&amp; <span class="branch-2 cbranch-no" title="branch not covered" >$reqMessage.worker.version == 1)</span> {
        //正式环境需要上报统计数据
&nbsp;
<span class="cstat-no" title="statement not covered" >        var masterSetInfo = '', masterContainer = '', slaveContainer = '', masterModuleName = '';</span>
...
&nbsp;
<span class="cstat-no" title="statement not covered" >        masterModuleName = masterConfig.get("taf.application.client.modulename", '');</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >        var enableSet = masterConfig.get("taf.application.enableset", 'n');</span>
<span class="cstat-no" title="statement not covered" >        var isDocker = masterConfig.get("taf.application.isdocker", 'n');</span>
<span class="cstat-no" title="statement not covered" >        if(enableSet == 'y' || enableSet == 'Y')</span>
        {
<span class="cstat-no" title="statement not covered" >            var setDivision = masterConfig.get("taf.application.setdivision", undefined);</span>
...
&nbsp;
<span class="cstat-no" title="statement not covered" >        if(isDocker == 'y' || isDocker == 'Y')</span>
        {
<span class="cstat-no" title="statement not covered" >            var container = masterConfig.get("taf.application.container", undefined);</span>
<span class="cstat-no" title="statement not covered" >            if(container)</span>
<span class="cstat-no" title="statement not covered" >                masterContainer = container;</span>
        }
&nbsp;
...
<span class="cstat-no" title="statement not covered" >        slaveContainer = $reqMessage.request.containerName || '';</span>
&nbsp;
...
&nbsp;
<span class="cstat-no" title="statement not covered" >        if(masterContainer)</span>
<span class="cstat-no" title="statement not covered" >            headers.masterContainer = masterContainer;</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >        if(slaveContainer)</span>
<span class="cstat-no" title="statement not covered" >            headers.slaveContainer = slaveContainer;</span>
&nbsp;
...
<span class="cstat-no" title="statement not covered" >            TafMonitor.stat.report(headers, TafMonitor.stat.TYPE.SUCCESS, 0);</span>
        }
&nbsp;
<span class="cstat-no" title="statement not covered" >        if ($iResultCode === 0 &amp;&amp; $reqMessage !== undefined) {</span>
<span class="cstat-no" title="statement not covered" >            TafMonitor.stat.report(headers, TafMonitor.stat.TYPE.SUCCESS, $reqMessage.costtime);</span>
        }
&nbsp;
<span class="cstat-no" title="statement not covered" >        if ($iResultCode !== 0 &amp;&amp; $reqMessage === undefined) {</span>
<span class="cstat-no" title="statement not covered" >            TafMonitor.stat.report(headers, TafMonitor.stat.TYPE.ERROR, 0);</span>
        }
&nbsp;
<span class="cstat-no" title="statement not covered" >        if ($iResultCode !== 0 &amp;&amp; $reqMessage !== undefined) {</span>
<span class="cstat-no" title="statement not covered" >            TafMonitor.stat.report(headers, $iResultCode === TafError.CLIENT.REQUEST_TIMEOUT?TafMonitor.stat.TYPE.TIMEOUT:TafMonitor.stat.TYPE.ERROR, $reqMessage.costtime);</span>
        }
    }
};</pre></td></tr>
</table></pre>
...
<div class='footer quiet pad2 space-top1 center small'>
  Code coverage
  generated by <a href="http://istanbul-js.org/" target="_blank">istanbul</a> at Wed Jul 10 2019 20:08:50 GMT+0800 (GMT+08:00)
</div>
</div>
