var Configure = require('../index').Config;

var configure = new Configure();
configure.parseFile('./config.conf', 'utf8');

var data = configure.data;

console.log('### get: taf.application.server.localip: ', configure.get('taf.application.server.localip'));
//console.log('### taf.log.format.a: ', configure.get('taf.log.format.a'));
//console.log('### taf.log.format.b: ', configure.get('taf.log.format.b'));
//console.log('### taf.log.format.aa.z: ', configure.get('taf.log.format.aa.z'));

//console.log('### get: taf.application.server.<TRom.ThemeStoreServer.ThemeStore1ObjAdapter>: ', configure.get("taf.application.server.<TRom.ThemeStoreServer.ThemeStore1ObjAdapter>"));
console.log('### getDomain: taf.application.server: ', configure.getDomain('taf.application.server'));
console.log('### getDomainValue: taf.application.server: ', configure.getDomainValue('taf.application.server'));

console.log('### getDomainLine: taf.application.server: ', configure.getDomainLine('taf.application.server',  [121]));

console.log('=== data ===', data.taf.application.server);
