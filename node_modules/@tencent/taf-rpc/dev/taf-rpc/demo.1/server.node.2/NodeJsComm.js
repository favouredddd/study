    ...
    switch (FuncName) {
        ...
        case 'secRequest' : {
            if (current.getRequestVersion() === TafStream.Wup.WUP_SIMPLE || current.getRequestVersion() === TafStream.Wup.WUP_COMPLEX) {
                ...
                wup.wupVersion = current.getRequestVersion();
                wup.decode(BinBuffer);
                var binRequest = wup.readBytes("binRequest", TafStream.BinBuffer);
                var binResponse = wup.readBytes("binResponse", TafStream.BinBuffer);
            } else {
                var is = new TafStream.JceInputStream(BinBuffer);
                var binRequest = is.readBytes(1, true, TafStream.BinBuffer);
                var binResponse = is.readBytes(2, false, TafStream.BinBuffer);
            }
            current.sendResponse = function (_ret, binResponse) {
                if (current.getRequestVersion() === TafStream.Wup.WUP_SIMPLE || current.getRequestVersion() === TafStream.Wup.WUP_COMPLEX) {
                    ...
                    wup.wupVersion = current.getRequestVersion();
                    wup.writeInt32("", _ret);
                    wup.writeBytes("binResponse", binResponse);
