...
var ReportObj = function(headers) {
	...
	this._StatMicMsgHead = new StatF.taf.StatMicMsgHead();
	...
	if (typeof headers.slaveContainer === 'string') {
		this._StatMicMsgHead.sSlaveContainer = headers.slaveContainer;
	}
	if (typeof headers.masterContainer === 'string') {
		this._StatMicMsgHead.sMasterContainer = headers.masterContainer;
	}
	this._StatMicMsgHead.iStatVer = 2;

	this._StatMicMsgBody = new StatF.taf.StatMicMsgBody();
	this._StatMicMsgBody.count = 0
	this._StatMicMsgBody.timeoutCount = 0;
	this._StatMicMsgBody.execCount = 0;

	this._StatMicMsgBody.intervalCount = new TafStream.Map(TafStream.Int32, TafStream.Int32);
...
var task = function() {
	var msgList = {
		fromClient : new TafStream.Map(StatF.taf.StatMicMsgHead, StatF.taf.StatMicMsgBody),
		fromServer : new TafStream.Map(StatF.taf.StatMicMsgHead, StatF.taf.StatMicMsgBody)
	};

	if (!client) {
		client = Taf.client.stringToProxy(StatF.taf.StatFProxy, exports.StatObj);
	}
...
exports.init = function(obj) {
	...
	if (obj) {
		...
		if (typeof obj === 'object') {
			exports.StatObj = obj.taf.application.client.stat;
			reportInterval = parseInt(obj.taf.application.client['report-interval']);
